...
std::pair<SenderDB, std::size_t> SenderDB::Load(const std::string& file_name) {
  STOPWATCH(sender_stopwatch, "SenderDB::Load");
  APSI_LOG_INFO("Start loading SenderDB");
  ...
  if (::fstat(fd, &st) < 0) {
    close(fd);
    throw runtime_error("fstat failed of file: ");
  }
  struct stat file_stat;
  ::stat(file_name.c_str(), &file_stat);
  auto addr = (unsigned char*)::mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
  unsigned char* data_ptr = addr;
  uint32_t data_len;
  uint32_t data_len_type_len = sizeof(data_len);
  memcpy(&data_len, data_ptr, data_len_type_len);
  data_len += data_len_type_len;
  APSI_LOG_DEBUG("first block data len: " << data_len);
  std::vector<unsigned char> in_data(data_len);
  memcpy(in_data.data(), data_ptr, data_len);
  data_ptr += data_len;
  ...
  // Copy over the OPRF key
  sender_db->oprf_key_.load(
    oprf_key_span_const_type(
  ...
  // Load all BinBundle data
  vector<vector<unsigned char>> bin_bundle_data;
  bin_bundle_data.resize(bin_bundle_count);
  // while (bin_bundle_count--) {
  //   bin_bundle_data.push_back(read_from_stream(in));
  // }
  std::vector<uint64_t> offset_info(bin_bundle_count);
  unsigned char* tmp_ptr = data_ptr;   // store current position
  for (size_t i = 0; i < bin_bundle_count; i++) {
    memcpy(&data_len, data_ptr, data_len_type_len);
    data_ptr += data_len_type_len;
    data_ptr += data_len;
    offset_info[i] = data_len + data_len_type_len;
  }
  data_ptr = tmp_ptr;  // restore
  std::vector<std::future<void>> futs;
  size_t thread_num = ThreadPoolMgr::GetThreadCount();
  thread_num = std::max<size_t>(thread_num, 15);
  size_t block_num = bin_bundle_count / thread_num;
  size_t rem_size = bin_bundle_count % thread_num;
  APSI_LOG_INFO("loading data use thread num " << thread_num << " "
      << "bin bundle size per thread: " << block_num);
  for (size_t i = 0; i < thread_num; i++) {
    futs.push_back(
      std::async(
        std::launch::async,
        [&, i]() {
          size_t index = i * block_num;
          for (size_t k = 0; k < block_num; k++) {
            uint64_t offset{0};
            for (int j = 0; j < index; j++) {
              offset += offset_info[j];
            }
            auto& data_len = offset_info[index];
            std::vector<unsigned char> item(data_len);
            memcpy(item.data(), data_ptr + offset, data_len);
            bin_bundle_data[index] = std::move(item);
            index++;
          }
        }
      ));
  ...
  if (rem_size) {
    futs.push_back(
      std::async(
        std::launch::async,
        [&]() {
          size_t index = thread_num * block_num;
  ...
  if (::munmap(addr, st.st_size) < 0) {
    close(fd);
    throw runtime_error("munmap failed of file");
  }
  close(fd);
  APSI_LOG_INFO("loading data from " << file_name << " finished");
  // Use multiple threads to recreate the BinBundles
  ThreadPoolMgr tpm;
  ...
  APSI_LOG_INFO("Finished loading SenderDB");
